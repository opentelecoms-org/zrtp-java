...
public class ContactImpl {
	...
	private static boolean fuzzyMatch(String number1, String number2) {
		final String suffix1 = number1.substring(number1.length()
				- ContactImpl.LENGHT_MIN, number1.length());
		final String suffix2 = number2.substring(number2.length()
				- ContactImpl.LENGHT_MIN, number2.length());
	 ...
	 */
	public static ContactImpl lookupByNumber(String aNumber) {
		return new ContactImpl(aNumber);
	}
	...
	public static boolean matchingNumbers(String number1, String number2) {
		...
		if (number1.length() < ContactImpl.LENGHT_MIN
				|| number2.length() < ContactImpl.LENGHT_MIN) {
			return number1.equals(number2);
		}
		if (ContactImpl.isInternationalFormat(number1)
				|| ContactImpl.isInternationalFormat(number2)) {
			return ContactImpl.fuzzyMatch(number1, number2);
		}
		return number1.equals(number2);
	}

	public static ContactImpl normalizeCaller(String caller) {
		ContactImpl c = null;
		int atSign = caller.indexOf('@');
		if (atSign != -1) {
			c = ContactImpl.lookupByNumber(caller.substring(0, atSign));
		} else {
			c = ContactImpl.lookupByNumber(caller);
		}
		return c;
	}
	...
	private ContactImpl(String number) {
